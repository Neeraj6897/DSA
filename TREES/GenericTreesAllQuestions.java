...
//MIRROR A GENERIC TREE: Create a mirror of the tree

    //Approach: Recursive approach is used here.
...
//FIND NODE TO ROOT PATH IN A GENERIC TREE
    ...
    public static ArrayList<Integer> nodeToRootPath(Node node, int data){
        ...
        for(Node child : node.children){
            ...
            // we just now need to return to root by adding path into ArrayList 
            if(res.size() > 0){
                res.add(node.data);
                return res;
            }
...
//ARE TREES SIMILAR IN *SHAPE* : (How to verify code: We don't have other tree to check, either create one to check or just check code.)
...
//FIND PREDECESSOR AND SUCCESSOR IN A GENERIC TREE
    ...
    static Node predecessor;            // We will use state variable to find our nodes.
    static Node successor;              // If state = 0, then if data is not equal to node.data then update predecessor to 
    static int state = 0;               // current node. If data equal to node.data then increase state and dont update predecessor        
...
//Other approach we can use is use extra space. One element before node.data in PREORDER traversal is predecessor and 
//after node.data is successor. So, we can use preorder traversal to find answer with extra space.


//=================================================================================

//CEIL AND FLOOR IN A GENERIC TREE FOR GIVEN NO.:
 
    static int ceil = Integer.MAX_VALUE; // CEIL means just larger element from given element
    static int floor = Integer.MIN_VALUE; // FLOOR means just smaller element from given element
    
    public static void ceilAndFloor(Node node, int data) {
        if(node.data > data) {  // It means among elements larger then data, we need to find smallest to find required ceil.
            if(node.data < ceil) {  // If smaller element found then we update ceil.
                ceil = node.data;
            }
        }

        if(node.data < data) { // If elements are smaller then data, then we need to find maximum among smaller elements for floor.
            if(node.data > floor) { //If greater then floor found then update floor.
                floor = node.data;
            }
        }
...
//Approach: We can use ceilAndFloor for this. By the definition of floor, floor of -∞ will be largest value and then floor of  
//largest value will be 2nd largest value and so on.

    public static int kthLargest(Node node, int k){
        
        //Use data variable to update floor value, initially it is kept +∞ to find largest element and then update according to 
        //floor value.
        int data = Integer.MAX_VALUE;
        
        //Loop till k times to find k'th largest element
        for(int i=0; i<k; i++){
            ceilAndFloor(node, data);
            //Update floor value to data to use this value in next iteration.
            data = floor;
            //Update floor back to original value
            floor = Integer.MIN_VALUE;   //floor is set to -∞ because floor means largest among smaller and we set -∞ for largest.     
        }
        return data;
    }
...
//NODE WITH MAXIMUM SUBTREE SUM: (Node with Sum of all children which is maximum)
    ...
    public static int maxSubtreeSum(Node node){
        ...
        //If sum of the subtree is maximun so far then we update the maxSum value and nodeData
...
//Answer is in the form nodeData@maxSum.


//======================================================================================================

//DIAMETER OF A GENERIC TREE: Diameter of tree is the maximum distance between two nodes in a tree.
//---------------------------

    //Approach: It can be calculated by finding maximum height and 2nd max. height nodes in the tree.
    ...
    public static int heightForDiameter(Node node){
        ...
        for(Node child : node.children){
            int ht = heightForDiameter(child);  //Height returned here will be the max. height for the child.
            
            if(ht > mxHeight){
                secndMxHeight = mxHeight;
                mxHeight = ht;
            }
            else if(ht > secndMxHeight){   // if height returned is not greater than mxHeight but greater than secndMxHeight.
                secndMxHeight = ht;
            }
        }
        
        
        //Here diameter will be equal to the sum of mxHeight and secndMxHeight + 2. This +2 is for both the mxHeight and 
        //secndMxHeight node connected to root.
        diameter = Math.max(diameter, mxHeight + secndMxHeight + 2);
